...
#define ZPMainThreadAssert() NSAssert([NSThread isMainThread], @"CCProgressHUD needs to be accessed on the main thread.");

CGFloat const CCProgressMaxOffset = 1000000.f;

static const CGFloat ZPDefaultPadding = 4.f;
static const CGFloat ZPDefaultLabelFontSize = 16.f;
static const CGFloat ZPDefaultDetailsLabelFontSize = 12.f;
...
+ (CCProgressHUD *)HUDForView:(UIView *)view {
    ...
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            CCProgressHUD *hud = (CCProgressHUD *)subview;
...
- (void)setupViews {
    ...
    ZPBackgroundView *backgroundView = [[ZPBackgroundView alloc] initWithFrame:self.bounds];
    ...
    ZPBackgroundView *bezelView = [ZPBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:ZPDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:ZPDefaultDetailsLabelFontSize];
    ...
    button.titleLabel.font = [UIFont boldSystemFontOfSize:ZPDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[ZPRoundProgressView class]];
    ...
    if (mode == CCProgressHUDModeIndeterminate) {
        ...
        [indicator removeFromSuperview];
        indicator = [[ZPBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
        ...
        if (!isRoundIndicator) {
            ...
            [indicator removeFromSuperview];
            indicator = [[ZPRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == CCProgressHUDModeAnnularDeterminate) {
            [(ZPRoundProgressView *)indicator setAnnular:YES];
        }
    } 
...
#pragma clang diagnostic pop
    ...
    } else if ([indicator isKindOfClass:[ZPRoundProgressView class]]) {
        ZPRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [ZPRoundProgressView appearanceWhenContainedIn:[CCProgressHUD class], nil];
#else
        appearance = [ZPRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[CCProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((ZPRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((ZPRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[ZPBarProgressView class]]) {
        ZPBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [ZPBarProgressView appearanceWhenContainedIn:[CCProgressHUD class], nil];
#else
        appearance = [ZPBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[CCProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((ZPBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((ZPBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    ZPBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? ZPDefaultPadding : 0.f;
...
@implementation ZPRoundProgressView
...
@implementation ZPBarProgressView
...
@implementation ZPBackgroundView
...
- (BOOL)dimBackground {
    ZPBackgroundView *backgroundView = self.backgroundView;
